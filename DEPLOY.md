...
The app deploys as a bare Linux binary behind Caddy, run by systemd. This
guide uses placeholders; substitute them once per environment:
...
Four things, side by side in `<app-dir>`:
...
No `.env` file: Loco does not read one. Environment variables live in the
systemd unit.
...
Afterwards `target/site` holds hashed files that the debug binary cannot
name, so run `cargo leptos build` (or the watch loop) before going back to
development.
...
### Check the port is free (on the server)
...
### Create the directory and upload (from the local terminal)
...
### Create the service (on the server)
...
# Secrets. Production refuses to boot without them; staging has placeholder
# defaults in its config file.
Environment=JWT_SECRET=<random string>
Environment=MAILER_HOST=<smtp host>
Environment=MAILER_USER=<smtp user>
Environment=MAILER_PASSWORD=<smtp password>
...
Add a block to `/etc/caddy/Caddyfile` (never replace a shared file):
...
Production usually adds a `www` redirect:
...
`compression` middleware stays off in the config. Validate, then reload
without downtime:
...
## Regular Deploy (from the local terminal)

Build `dist/` as above, then one chain; any failing step aborts it, so a
failed build never touches the server:

```bash
ssh <user>@<host> "systemctl stop <unit>" &&
scp dist/app <user>@<host>:<app-dir>/app &&
scp dist/hash.txt <user>@<host>:<app-dir>/hash.txt &&
ssh <user>@<host> "rm -rf <app-dir>/site" &&
scp -r dist/site <user>@<host>:<app-dir>/ &&
ssh <user>@<host> "find <app-dir>/site -name '.DS_Store' -delete" &&
scp config/<env>.yaml <user>@<host>:<app-dir>/config/ &&
ssh <user>@<host> "systemctl start <unit> && systemctl status <unit> --no-pager"
```

`site/` is removed before upload so files deleted locally do not linger on
the server, and macOS Finder files that cargo-leptos copied from `public/`
are deleted after the upload. Only the one config file is uploaded: Loco
reads the file named by `LOCO_ENV` and nothing else.
...
LOCO_ENV=<env> BINDING=127.0.0.1 PORT=<port> LEPTOS_OUTPUT_NAME=app LEPTOS_SITE_ROOT=site LEPTOS_SITE_PKG_DIR=pkg LEPTOS_ENV=PROD ./app start
```
...
```bash
ssh <user>@<host> 'cd <app-dir> && for p in / /robots.txt $(ls site/pkg | grep -v "\.ts$" | sed "s|^|/pkg/|"); do printf "%-40s " "$p"; curl -s -o /dev/null -w "%{http_code}\n" "http://127.0.0.1:<port>$p"; done && curl -s http://127.0.0.1:<port>/ | grep -o "href=\"/pkg/[^\"]*\""'
```

All 200, and the `href` values printed last must be among the hashed files
listed above them (the page links what the deploy shipped). Then open
`https://<domain>` with the browser console open: the page renders, no
errors, and the wasm and JS requests show in the network tab.
...
- Cloudflare injects `Strict-Transport-Security` and
  ...
  server to see what the app itself sends.
...
HSTS go-live checklist for the apex domain, once the site answers over https
and `www` redirects: (1) set the zone's HSTS (SSL/TLS, Edge Certificates) to
max-age 12 months with "Apply to subdomains" and "Preload" on, matching the
origin header in `config/production.yaml`; (2) submit the apex domain at
https://hstspreload.org; (3) from then on every subdomain must be https, and
